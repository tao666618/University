...
public Stat getStat(Stat stat){
	...
	try {
		...
		while (rs.next()) {
			...
			stat.setRead_BL(rs.getInt("read_BL"));/*yezi2013-5_15*/
			stat.setRead_FS(rs.getInt("read_FS"));/*yezi2013-5_15*/
		}
	} catch (Exception e) {
		e.printStackTrace();
	} finally {
...
public boolean updateStatDBbean(Stat stat) {
	boolean blnrec = true;
	String strSql = "update tb_stat set radio_BL =?,more_BL=?,judge_BL=?,read_BL=?,radio_FS=? "
			+ ",more_FS = ?,judge_FS = ?,read_FS=?,exam_time = ? where id = ?";/*yezi2013-5-19*/
	...
	try {
		...
		pstmt.setInt(4, stat.getRead_BL());/*yezi2013-5-19*/
		pstmt.setInt(5, stat.getRadio_FS());
		pstmt.setInt(6, stat.getMore_FS());
		pstmt.setInt(7, stat.getJudge_FS());/*yezi2013-5-19*/
		pstmt.setInt(8, stat.getRead_FS());
		pstmt.setInt(9, stat.getExam_time());
		pstmt.setInt(10, stat.getId());
		pstmt.executeUpdate();
	} catch (Exception e) {
		e.printStackTrace();
		blnrec = false;
