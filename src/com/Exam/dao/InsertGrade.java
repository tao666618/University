...
public boolean setGradeDBbean(Grade  grade) {
	...
	try {
		...
		pstmt.setInt(6, grade.getReadResult());
		pstmt.setInt(7, grade.getBatsisResult());
		pstmt.setString(8,grade.getDate());
		pstmt.executeUpdate();
	} catch (Exception e) {
		e.printStackTrace();
		blnrec = false;
...
public boolean updateGradeDBbean(Grade  grade) {
	boolean blnrec = true;
	String strSql = "update tb_grade set UserName=?,radioResult=?,fullResule=?,esitResult=?,readResult=?,batsisResult=?,date=?"+
					"where id=?";
	System.out.println(strSql);
	PreparedStatement pstmt = null;
	try {
