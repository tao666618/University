        ...
        if (list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                ...
                model.addRow(new Object[]{
                        user.getId(), user.getUserType() == 1 ? "管理员" : "用户", ChDeal.toChinese(user.getUserName()),
            ...
            public void mouseClicked(MouseEvent e) {
                if (e.getClickCount() == 1) { // 双击事件
                    ...
                    if (column == 4) { // 如果点击的是“是否考试”列
                        ...
                        user.setSubject((String) model.getValueAt(row, 3));

                        user.setId(id);
                        user.setN(findGrade.getNForIdAndHavaIn(user.getId(), user.getHaveIn(), user.getSubject()));
                    ...
                    if (column == 3) {
                        String currentValue = (String) model.getValueAt(row, column);
                        String haveln = (String) model.getValueAt(row, 4);
                        user.setHaveIn(haveln.equals("是") ? 1 : 0);
                        // 获取修改后的值
